...
class MarkdownGenerator extends BaseGenerator_js_1.BaseGenerator {
    ...
    renderAttributeList(meta) {
        ...
        if (meta.width)
            parts.push(`width=${String(meta.width).replace(/[{}\s]+/g, '')}`);
        if (meta.align)
            parts.push(`align=${String(meta.align).replace(/[{}\s]+/g, '')}`);
        return `{${parts.join(' ')}}`;
    }
    ...
    async generate() {
        ...
        if (this.ast.metadata) {
            ...
            if (this.ast.metadata.title)
                output += `title: ${JSON.stringify(this.ast.metadata.title)}\n`;
            if (this.ast.metadata.author)
                output += `author: ${JSON.stringify(this.ast.metadata.author)}\n`;
            ...
            if (this.ast.metadata.description)
                output += `description: ${JSON.stringify(this.ast.metadata.description)}\n`;
