...
export class ContextBuilder {
   ...
   */
  public buildContext(chunks: RetrievalResult[]): { contextText: string; deduplicated: RetrievalResult[] } {
    if (chunks.length === 0) {
      return { contextText: '', deduplicated: [] };
    }

    // 1. Remove duplicate chunks by chunkId
    const uniqueMap = new Map<string, RetrievalResult>();
    ...
    logger.debug(`[Context Builder] Compiled logical context from ${deduplicated.length} chunks.`);
    return { contextText, deduplicated };
  }
}
