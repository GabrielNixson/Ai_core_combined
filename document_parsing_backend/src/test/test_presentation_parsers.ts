...
import { PptxParser } from '../parsers/pptx/pptxParser';
...
// Test file paths
const pptxValidPath = path.join(uploadsDir, 'test_presentation.pptx');
const pptxEmptyPath = path.join(uploadsDir, 'test_empty.pptx');
const pptxCorruptedPath = path.join(uploadsDir, 'test_corrupted.pptx');

async function createMockPptx(): Promise<Buffer> {
  const zip = new JSZip();
...
async function runTests() {
  ...
  try {
    assert(
      ParserFactory.getParser(DocumentType.PPTX) instanceof PptxParser,
      'Factory maps PPTX to PptxParser'
    );
  } catch (err) {
  ...
  try {
    ...
    fs.writeFileSync(pptxValidPath, buffer);
    ...
    assert(doc.documentType === DocumentType.PPTX, 'DocumentType is PPTX');
    ...
    assert(
      slide1Section?.content[2]?.content.includes('Presenter notes:'),
    ...
    assert(
      slide1MetaBlock?.content.content.includes('Verify connection'),
      'Metadata content includes full slide text'
    );
  ...
  try {
    fs.writeFileSync(pptxEmptyPath, '');
    const parser = new PptxParser();
    await parser.parse({
    ...
    assert(
      err instanceof BadRequestError && err.message.includes('Empty PPTX file'),
      'Threw BadRequestError on empty file'
    );
  }

  try {
    fs.writeFileSync(pptxCorruptedPath, 'Not a zip at all.');
    const parser = new PptxParser();
    await parser.parse({
    ...
    assert(
      err instanceof BadRequestError && err.message.includes('Corrupted or invalid PPTX file'),
      'Threw BadRequestError on corrupted file'
    );
  }

  // Cleanup files
  try {
    fs.unlinkSync(pptxValidPath);
    fs.unlinkSync(pptxEmptyPath);
    fs.unlinkSync(pptxCorruptedPath);
  } catch (_) { }
