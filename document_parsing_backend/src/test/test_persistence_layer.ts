...
async function runTests() {
  ...
  try {
    ...
    await DocumentModel.deleteMany({ documentId: testDocId });
    ...
    await DocumentModel.deleteOne({ documentId: 'default-version-doc' });

    // Test 3: updateStatus & findById
    await docRepo.updateStatus(testDocId, DocumentStatus.PARSED, {
      markdownPath: '/uploads/markdown/test.md',
    });

    const updatedDoc = await docRepo.findByDocumentId(testDocId);
    assert(updatedDoc?.status === DocumentStatus.PARSED, 'updateStatus transitioned status correctly');
    ...
    const docsByStatus = await docRepo.findByStatus(DocumentStatus.PARSED);
    ...
    const rawLooked = await DocumentModel.findOne({ documentId: testDocId }).lean().exec();
    ...
    await DocumentModel.updateOne({ documentId: testDocId }, { $set: { isDeleted: false } }).exec();

    // Test 6: Chunk Repository bulk insert & pointers
    const testChunks: Partial<DocumentChunk>[] = [
      {
        chunkId: 'chunk-1',
        documentId: testDocId,
        chunkIndex: 0,
    ...
    const indexes = await DocumentModel.db.db?.collection('documents').indexes();
    const chunkIndexes = await ChunkModel.db.db?.collection('documentchunks').indexes();
    ...
    try {
      ...
      await docRepo.updateStatus(testDocId, DocumentStatus.CHUNKING, {}, session);
      ...
      assert(txDoc?.status !== DocumentStatus.CHUNKING, 'Transaction aborted: Document status rolled back successfully');
