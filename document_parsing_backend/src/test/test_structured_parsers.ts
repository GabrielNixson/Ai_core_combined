...
import { JsonParser } from '../parsers/json/jsonParser';
import { XmlParser } from '../parsers/xml/xmlParser';
...
const jsonSimplePath = path.join(uploadsDir, 'test_simple.json');
const jsonNestedPath = path.join(uploadsDir, 'test_nested.json');
const jsonArrayPath = path.join(uploadsDir, 'test_array.json');
const jsonInvalidPath = path.join(uploadsDir, 'test_invalid.json');

const xmlSimplePath = path.join(uploadsDir, 'test_simple.xml');
const xmlNestedPath = path.join(uploadsDir, 'test_nested.xml');
const xmlAttributesPath = path.join(uploadsDir, 'test_attributes.xml');
const xmlRepeatedPath = path.join(uploadsDir, 'test_repeated.xml');
const xmlInvalidPath = path.join(uploadsDir, 'test_invalid.xml');
...
async function runTests() {
  ...
  try {
    assert(
      ParserFactory.getParser(DocumentType.JSON) instanceof JsonParser,
      'Factory maps JSON to JsonParser'
    );
    assert(
      ParserFactory.getParser(DocumentType.XML) instanceof XmlParser,
      'Factory maps XML to XmlParser'
    );
  } catch (err) {
  ...
  try {
    fs.writeFileSync(jsonSimplePath, jsonSimplePayload);
    ...
    assert(doc.documentType === DocumentType.JSON, 'DocumentType is JSON');
    ...
    assert(
      doc.sections[0]?.content[0]?.content.name === 'Energy Meter',
      'Extracted simple string value'
    );
  } catch (err) {
  ...
  try {
    fs.writeFileSync(jsonNestedPath, jsonNestedPayload);
    const parser = new JsonParser();
    const doc = await parser.parse({
  ...
  try {
    fs.writeFileSync(jsonArrayPath, jsonArrayPayload);
    const parser = new JsonParser();
    const doc = await parser.parse({
    ...
    assert(
      doc.sections[0]?.content[0]?.content[0].address === 40001,
      'Array first object address matches'
    );
  } catch (err) {
  ...
  try {
    fs.writeFileSync(jsonInvalidPath, jsonInvalidPayload);
    const parser = new JsonParser();
    await parser.parse({
    ...
    assert(
      err instanceof BadRequestError && err.message.includes('Invalid JSON syntax'),
      `Correctly threw BadRequestError for invalid JSON syntax: "${err.message}"`
    );
  }

  // Test 6: XML Parser - Simple XML
  try {
    fs.writeFileSync(xmlSimplePath, xmlSimplePayload);
    ...
    assert(doc.documentType === DocumentType.XML, 'DocumentType is XML');
    ...
    assert(
      doc.sections[0]?.content[0]?.content.name === 'Energy Meter',
      'Parsed child tag value correctly'
    );
  } catch (err) {
  ...
  try {
    fs.writeFileSync(xmlNestedPath, xmlNestedPayload);
    const parser = new XmlParser();
    const doc = await parser.parse({
  ...
  try {
    fs.writeFileSync(xmlAttributesPath, xmlAttributesPayload);
    const parser = new XmlParser();
    const doc = await parser.parse({
    ...
    assert(
      doc.sections[0]?.content[0]?.content.attributes.id === '101',
      'Preserved root node attributes in ContentBlock'
    );
    assert(
      doc.sections[0]?.content[0]?.content.register.attributes.address === '40001',
      'Preserved child node attributes in ContentBlock'
    );
    assert(
      doc.sections[0]?.content[0]?.content.register.value === 'Voltage',
      'Preserved child value in ContentBlock'
    );
  } catch (err) {
  ...
  try {
    fs.writeFileSync(xmlRepeatedPath, xmlRepeatedPayload);
    const parser = new XmlParser();
    const doc = await parser.parse({
    ...
    assert(
      doc.sections[0]?.content[0]?.content[0] === 'Voltage',
      'First element string matches'
    );
  } catch (err) {
  ...
  try {
    fs.writeFileSync(xmlInvalidPath, xmlInvalidPayload);
    const parser = new XmlParser();
    await parser.parse({
    ...
    assert(
      ...
      `Correctly threw BadRequestError for invalid XML syntax/unclosed tags: "${err.message}"`
    );
  }

  // Cleanup files
  try {
    fs.unlinkSync(jsonSimplePath);
    fs.unlinkSync(jsonNestedPath);
    fs.unlinkSync(jsonArrayPath);
    fs.unlinkSync(jsonInvalidPath);
    fs.unlinkSync(xmlSimplePath);
    fs.unlinkSync(xmlNestedPath);
    fs.unlinkSync(xmlAttributesPath);
    fs.unlinkSync(xmlRepeatedPath);
    fs.unlinkSync(xmlInvalidPath);
  } catch (_) { }
