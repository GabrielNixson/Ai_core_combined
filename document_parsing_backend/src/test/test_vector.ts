...
async function runTests() {
  ...
  try {
    const repo = new VectorRepository(new QdrantVectorProvider());
    ...
    // Clean up collection
    const provider = (repo as any).provider;
  ...
  try {
    ...
    await docRepo.create({
      ...
      extension: 'txt',
      size: 100,
      status: DocumentStatus.EMBEDDING_COMPLETED,
      processingVersion: 1,
    });
    ...
    // 3. Queue a sync job
    const queue = VectorQueue.getInstance();
    ...
    const worker = new VectorSyncWorker();

    // Inject mock Qdrant VectorProvider collection setup inside the service
    const mockProvider = new QdrantVectorProvider();
    (worker as any).syncService = new VectorSyncService(new VectorRepository(mockProvider));
    ...
    assert(doc?.status === DocumentStatus.INDEXED, 'Document status updated to INDEXED');
    ...
    assert(chunks.every(c => c.vectorSyncStatus === VectorSyncStatus.SYNCED), 'All chunks set to SYNCED vector status');
    ...
    const uuid1 = uuidv5('chunk-sync-1', NAMESPACE);
  ...
  try {
    ...
    class FailingProvider extends QdrantVectorProvider {
      async upsertVectors(): Promise<void> { throw new Error('Qdrant Timeout'); }
    }
    const failRepo = new VectorRepository(new FailingProvider());
    (worker as any).syncService = new VectorSyncService(failRepo);
    ...
    assert(retryingChunk?.vectorSyncStatus === VectorSyncStatus.RETRYING, 'Chunk status updated to RETRYING');
    ...
    assert(failedChunk?.vectorSyncStatus === VectorSyncStatus.FAILED, 'Chunk vectorSyncStatus set to FAILED after final attempt');

    const doc = await docRepo.findByDocumentId(testDocId);
    assert(doc?.status === DocumentStatus.FAILED, 'Document status set to FAILED');
