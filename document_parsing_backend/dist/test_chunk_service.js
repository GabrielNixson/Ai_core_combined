...
const tokenEstimator_1 = require("./chunking/utils/tokenEstimator");
...
async function runTests() {
    ...
    try {
        ...
        const tokens = tokenEstimator_1.TokenEstimator.estimateTokens(text);
        assert(tokens === Math.ceil(38 / 4), `Estimated correct tokens for simple text (chars: 38, tokens: ${tokens})`);
        assert(tokenEstimator_1.TokenEstimator.estimateTokens('') === 0, 'Empty string yields 0 tokens');
    }
    catch (err) {
    ...
    try {
        const service = new chunkGeneration_service_1.ChunkGenerationService({ mergeSmallChunks: false });
    ...
    try {
        const service = new chunkGeneration_service_1.ChunkGenerationService({
        ...
        assert(sec1TextChunks[0]?.content.includes('Danger High Voltage\n\nDo not touch internal terminals.\n\n- Wear safety gloves\n- Use insulated tools'), 'Text conjoined using double newlines and formatted lists');
    ...
    try {
        ...
        const repository = new chunk_repository_1.ChunkRepository();
        const service = new chunkGeneration_service_1.ChunkGenerationService();
        const chunks = service.generateChunks(testDoc);
